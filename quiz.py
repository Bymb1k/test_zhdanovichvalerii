    ...
    def flatten(lst: List[Any]) -> List[int]:
        ...
        for item in lst:
            if isinstance(item, int):
                ...
                flat_list.extend(flatten(item))  # рекурсивно разворачиваем вложенные списки
        return flat_list

    flat_numbers = flatten(nested_list) # Плоский список всех чисел
    
    filtered = {num for num in flat_numbers if num > min_value} # Фильтрация по min_value и удаление дубликатов
    
    return sorted(filtered) # Сортировка и возврат результата
...
def analyze_text_complexity(text: str) -> Dict[str, float]:
    ...
    cleaned_text = re.sub(r'[.,!?:;"“”«»]', '', text) # Удаляем знаки препинания, кроме дефисов внутри слов

    cleaned_text = cleaned_text.lower() # Приводим к нижнему регистру

    words = cleaned_text.split() # Разделяем на слова
    total_words = len(words)

    unique_words = len(set(words))  # Уникальные слова

    lexical_diversity = unique_words / total_words if total_words > 0 else 0.0 # Лексическое разнообразие

    total_letters = 0 # Сумма букв в словах (удаляем всё, кроме букв)
    for word in words:
...
def group_by_first_letter(strings: List[str]) -> Dict[str, List[str]]:
    ...
    for s in strings:
        ...
        if first_char.isalpha():  # Проверяем, что первый символ — буква (русская или латинская)
            result[first_char].append(s)

    for key in result: # Сортировка строк внутри каждой группы
        result[key].sort()

    return dict(result)


# === Задача 4: Симуляция банковской очереди ===
def simulate_bank_queue(arrival_times: List[int], service_time: int) -> float:
    """
    Симулирует работу одного банковского кассира и вычисляет среднее время ожидания клиентов.

    Параметры:
    - `arrival_times`: Список времен прихода клиентов (целые числа, неотрицательные, отсортированы по возрастанию).
    - `service_time`: Фиксированное время (целое число > 0), необходимое для обслуживания одного клиента.

    Правила симуляции:
    - Кассир свободен в момент времени 0.
    - Клиенты приходят в моменты времени, указанные в `arrival_times`.
    - Если клиент приходит и кассир свободен, обслуживание начинается немедленно в момент прихода.
    - Если клиент приходит, а кассир занят, клиент встает в очередь.
    - Кассир обслуживает клиентов по порядку их прихода.
    - Кассир начинает обслуживать следующего клиента из очереди сразу же, как только освобождается.
    - Время ожидания клиента = (Время начала обслуживания) - (Время прихода клиента).

    Задача:
    1. Рассчитать общее время ожидания для всех клиентов.
    2. Вернуть среднее время ожидания (общее время ожидания / количество клиентов).
    3. Если список `arrival_times` пуст, вернуть 0.0.

    Пример:
    simulate_bank_queue([0, 1, 4], 3)
    - Клиент 1 (приход 0): Кассир свободен. Начало обслуживания 0. Конец 3. Ожидание 0 - 0 = 0. Кассир свободен в 3.
    - Клиент 2 (приход 1): Кассир занят до 3. Начало обслуживания max(1, 3) = 3. Конец 3 + 3 = 6. Ожидание 3 - 1 = 2. Кассир свободен в 6.
    - Клиент 3 (приход 4): Кассир занят до 6. Начало обслуживания max(4, 6) = 6. Конец 6 + 3 = 9. Ожидание 6 - 4 = 2. Кассир свободен в 9.
    Среднее ожидание: (0 + 2 + 2) / 3 = 4 / 3
    вернет 1.333...
    simulate_bank_queue([2, 3, 4], 2)
    - Клиент 1 (приход 2): Начало 2. Конец 4. Ожидание 0. Кассир свободен в 4.
    - Клиент 2 (приход 3): Начало max(3, 4) = 4. Конец 6. Ожидание 4 - 3 = 1. Кассир свободен в 6.
    - Клиент 3 (приход 4): Начало max(4, 6) = 6. Конец 8. Ожидание 6 - 4 = 2. Кассир свободен в 8.
    Среднее ожидание: (0 + 1 + 2) / 3 = 1.0
    вернет 1.0
    simulate_bank_queue([], 5) вернет 0.0
    """
    # Ваше решение здесь
from typing import List

def simulate_bank_queue(arrival_times: List[int], service_time: int) -> float:
    if not arrival_times: # Если клиентов нет — среднее ожидание 0.0
        return 0.0

    total_wait_time = 0
    free_at = 0  # время, когда кассир будет свободен

    for arrival in arrival_times: # Обрабатываем каждого клиента по порядку
        start_service = max(arrival, free_at)
        wait_time = start_service - arrival
        total_wait_time += wait_time
        free_at = start_service + service_time

    return total_wait_time / len(arrival_times)
...
def find_upcoming_birthdays(contacts: Dict[str, str], days_limit: int) -> List[Dict[str, Any]]:
    ...
    for name, birth_str in contacts.items():
        birth_date = datetime.datetime.strptime(birth_str, "%Y-%m-%d").date() # Преобразуем строку в дату рождения

        try:
            next_birthday = birth_date.replace(year=today.year)
        except ValueError:
            next_birthday = datetime.date(today.year, 2, 28) # Обработка 29 февраля в невисокосный год → считаем днём рождения 28 февраля

        if next_birthday < today: # Если день рождения в этом году уже прошёл, переносим на следующий год
            try:
                next_birthday = birth_date.replace(year=today.year + 1)
            except ValueError:
                next_birthday = datetime.date(today.year + 1, 2, 28) # Обработка 29 февраля в невисокосном следующем году

        days_until = (next_birthday - today).days # Вычисляем количество дней до следующего дня рождения

        if 0 <= days_until < days_limit: # Если в пределах лимита — добавляем контакт в результат
            result.append({
                'name': name,
    ...
    result.sort(key=lambda x: x['days_until']) # Сортируем по возрастанию количества дней до дня рождения
    return result
...
def find_path_in_grid(grid: List[List[int]], start: Tuple[int, int], end: Tuple[int, int]) -> Optional[List[Tuple[int, int]]]:
    def is_valid(x, y): # Проверка на выход за пределы сетки или на стену
        return 0 <= x < len(grid) and 0 <= y < len(grid[0]) and grid[x][y] == 0

    if not (is_valid(start[0], start[1]) and is_valid(end[0], end[1])): # Проверка на валидность стартовой и конечной точки
        return None

    if start == end: # Если старт и конечная точка совпадают
        return [start]
    
    directions = [(1, 0), (-1, 0), (0, 1), (0, -1)] # Направления для движения: вниз, вверх, вправо, влево
    
    queue = [(start, [start])]  # Очередь для BFS (сначала стартовая точка, путь - это список с первой точкой)
    visited = set()  # множество посещенных ячеек
    visited.add(start)

    while queue:
        current, path = queue.pop(0)  # извлекаем текущую ячейку из очереди
        x, y = current

        for dx, dy in directions: # Проверяем все 4 возможных направления
            nx, ny = x + dx, y + dy
            if is_valid(nx, ny) and (nx, ny) not in visited:
                if (nx, ny) == end:
                    return path + [(nx, ny)]  # нашли путь до конечной точки
                queue.append(((nx, ny), path + [(nx, ny)]))  # добавляем в очередь
                visited.add((nx, ny))
...
def generate_secure_password(length: int, use_digits: bool, use_special_chars: bool) -> str:
    # Минимальная длина пароля в зависимости от выбранных опций
    min_length = 2  # для маленькой и большой буквы
    if use_digits:
        min_length += 1  # +1 для цифры
    if use_special_chars:
        min_length += 1  # +1 для спецсимвола

    if length < min_length:  # Проверка минимальной длины
        raise ValueError(f"Минимальная длина пароля должна быть {min_length}")

    lowercase = string.ascii_lowercase  # Строчные буквы
    uppercase = string.ascii_uppercase  # Прописные буквы
    digits = string.digits  # Цифры
    special_chars = "!@#$%^&*()-_=+[]{}|;:',.<>?"  # Спецсимволы

    password_chars = [random.choice(lowercase), random.choice(uppercase)] # Список символов для пароля
    ...
    all_chars = lowercase + uppercase # Собираем все возможные символы
    if use_digits:
        all_chars += digits
    if use_special_chars:
        all_chars += special_chars

    remaining_length = length - len(password_chars) # Генерация оставшихся случайных символов
    password_chars += random.choices(all_chars, k=remaining_length)

    random.shuffle(password_chars) # Перемешиваем символы для случайности
...
def merge_intervals(intervals: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    ...
    intervals.sort(key=lambda x: x[0]) # Сортируем интервалы по началу интервала (по первому элементу кортежа)
    
    merged = [] # Список для хранения объединенных интервалов
    
    merged.append(intervals[0]) # Инициализируем первый интервал в merged
    
    for current in intervals[1:]: # Проходим по оставшимся интервалам
        last_merged = merged[-1] # Проверяем, перекрывается ли текущий интервал с последним объединенным
        
        if current[0] <= last_merged[1]: # Если текущий интервал перекрывается или соприкасается с последним, сливаем их
            merged[-1] = (last_merged[0], max(last_merged[1], current[1]))
        else:
            merged.append(current) # Иначе добавляем текущий интервал как новый
...
def trap_rain_water_simple(heights: List[int]) -> int:
    ...
    max_left = [0] * n # Создаем массивы для хранения максимальных высот слева и справа
    max_right = [0] * n
    
    max_left[0] = heights[0] # Заполняем max_left
    for i in range(1, n):
        max_left[i] = max(max_left[i-1], heights[i])
    
    max_right[n-1] = heights[n-1]  # Заполняем max_right
    for i in range(n-2, -1, -1):
        max_right[i] = max(max_right[i+1], heights[i])
    
    total_water = 0 # Рассчитываем количество воды, которую можно удержать
    for i in range(n):
...
def validate_data_structure(data: Any, schema: Dict[str, type]) -> bool:
    if not isinstance(data, dict): # Проверяем, является ли data словарем
        return False
    
    for key in schema: # Проверяем, что все ключи из schema присутствуют в data
        if key not in data:
            return False
        
        if not isinstance(data[key], schema[key]): # Проверяем, что тип данных соответствует типу в schema
            return False
...
def decode_caesar_variant(encoded_text: str, key_word: str) -> str:
    ...
    # Ваше решение здесь
def decode_caesar_variant(encoded_text: str, key_word: str) -> str:
    decoded_text = []  # Список для собранного расшифрованного текста
    key_index = 0  # Индекс для обхода ключевого слова

    for char in encoded_text:
        if char.isalpha():  # Только буквы подлежат расшифровке
            shift = ord(key_word[key_index % len(key_word)]) - ord('a')  # сдвиг по ключу
            if char.islower():  # Если буква в нижнем регистре
                new_char = chr((ord(char) - ord('a') - shift + 26) % 26 + ord('a'))
            elif char.isupper():  # Если буква в верхнем регистре
                new_char = chr((ord(char) - ord('A') - shift + 26) % 26 + ord('A'))
            decoded_text.append(new_char)
            key_index += 1  # Переходим к следующей букве в ключе
        else:
            decoded_text.append(char)

    return ''.join(decoded_text)  # Собираем финальный текст в строку и возвращаем
...
def flatten_dict(nested_dict: Dict[str, Any], separator: str = '_') -> Dict[str, Any]:
    ...
    for key, value in nested_dict.items():
        new_key = str(key)  # ключ текущего уровня

        if isinstance(value, dict): # Если значение - это словарь, рекурсивно обрабатываем его
            flat_map.update(flatten_dict(value, separator=separator)) # Рекурсивный вызов для вложенного словаря, добавляем текущий ключ как префикс
        else:
            flat_map[new_key] = value # Если значение не словарь, добавляем пару (путь, значение)
...
def find_lonely_numbers(numbers: List[int]) -> List[int]:
    count = Counter(numbers)# Подсчитываем частоту каждого числа
    
    number_set = set(numbers) # Преобразуем список в множество для быстрого поиска соседей
    
    lonely_numbers = [] # Список для хранения "одиноких" чисел
    
    for num in count:
        if count[num] == 1 and (num - 1 not in number_set) and (num + 1 not in number_set):
            lonely_numbers.append(num)
    
    return sorted(lonely_numbers)
...
def run_length_encode(data: str) -> str:
    ...
    # Ваше решение здесь
def run_length_encode(data: str) -> str:
    if not data: # Если строка пуста, сразу возвращаем пустую строку
        return ""
    
    result = []
    count = 1  
    for i in range(1, len(data)):  # Пройдем по строке начиная с 1-го символа
        if data[i] == data[i - 1]:  # Если текущий символ такой же, как предыдущий, увеличиваем счетчик
            count += 1
        else: 
            result.append(data[i - 1] + (str(count) if count > 1 else ''))  # Добавляем символ и его количество (если больше 1)
            count = 1  # Сбрасываем счетчик для следующего символа
    
    result.append(data[-1] + (str(count) if count > 1 else '')) # Добавляем последний символ
    
    return ''.join(result)  # Собираем результат в строку
...
def run_length_decode(encoded_data: str) -> str:
    ...
    # Ваше решение здесь
def run_length_decode(encoded_data: str) -> str:
    if not encoded_data: # Если строка пустая — сразу возвращаем пустую строку
        return ""

    decoded = []
    i = 0 # Индекс текущего символа в закодированной строке
    while i < len(encoded_data):
        ...
        while i < len(encoded_data) and encoded_data[i].isdigit():
            ...
            i += 1
        count = int(count_str) if count_str else 1 # Если цифры были — преобразуем в int, иначе считаем как 1 повтор
        decoded.append(char * count) # Добавляем в результат символ, повторённый нужное количество раз
...
def find_anagram_groups(words: List[str]) -> List[List[str]]:
    ...
    # Ключ — отсортированное слово в нижнем регистре (например, "eat" -> "aet"),
    # значение — список слов, соответствующих этому ключу.
    groups: Dict[str, List[str]] = defaultdict(list)

    for word in words:
        if word == "":
            continue  # Пропускаем пустые строки
        key = ''.join(sorted(word.lower())) # Приводим к нижнему регистру и сортируем буквы в слове, чтобы получить ключ
        groups[key].append(word) # Добавляем оригинальное слово (с сохранением регистра) в соответствующую группу

    result = [sorted(anagram_list) for anagram_list in groups.values()] # Формируем итоговый список: внутри каждой группы сортируем слова по алфавиту
    return result
...
def moving_average(data: List[Union[int, float]], window_size: int) -> List[Optional[float]]:
    if window_size <= 0 or window_size > len(data): # Обрабатываем случаи, когда окно невалидное
    ...
    for i in range(len(data)):
        if i < window_size - 1: 
            result.append(None) # Пока не набралось достаточно элементов для окна — добавляем None
        else:
            window = data[i - window_size + 1:i + 1] # Срез окна: от (i - window_size + 1) до i включительно
...
def sum_digits_until_single(number: int) -> int:
    ...
    # Ваше решение здесь
def sum_digits_until_single(number: int) -> int:
    if number < 10: # Базовый случай: если число меньше 10, оно уже однозначное
        return number

    digit_sum = 0 # Считаем сумму цифр числа
    while number > 0:
        digit_sum += number % 10  # Получаем последнюю цифру
        number //= 10             # Удаляем последнюю цифру

    return sum_digits_until_single(digit_sum) # Рекурсивно вызываем ту же функцию с полученной суммой
...
def rotate_matrix_inplace(matrix: List[List[int]]) -> None:
    n = len(matrix)
    # Проверка на пустую или не квадратную матрицу
    if n == 0 or any(len(row) != n for row in matrix):
        return  

    for i in range(n): # Транспонируем матрицу
        for j in range(i + 1, n):
            # Меняем местами элементы [i][j] и [j][i]
            matrix[i][j], matrix[j][i] = matrix[j][i], matrix[i][j]

    for row in matrix: # Отражаем каждую строку по горизонтали
        row.reverse()